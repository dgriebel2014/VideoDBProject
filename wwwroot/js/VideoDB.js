...
export class VideoDB {
     ...
     */
    async confirmJsonOffsets(storeName, maxKeys = 25) {
        ...
        if (storeMeta.dataType !== "JSON" || !storeMeta.sortDefinition?.length) {
            console.warn(`Store "${storeName}" does not appear to be JSON or doesn't have any sortDefinition. Aborting offset check.`);
            return;
        }
        ...
        if (!offsetsMeta) {
            console.warn(`Offsets store "${offsetsStoreName}" not found. Make sure you createdObjectStore(...) with a sortDefinition.`);
            return;
        }
        ...
        if (totalFields === 0) {
            console.warn(`No sort fields found in ${storeName} sortDefinition. Nothing to verify.`);
            return;
        }
        ...
        console.log(`Checking up to ${keysToCheck.length} key(s) (out of ${allKeys.length}) and verifying each field offset.\n`);
        ...
        for (let i = 0; i < keysToCheck.length; i++) {
            ...
            const offsetArray = offsetRecords[i];
            ...
            if (jsonObject == null || offsetArray == null) {
                console.warn(`[${storeName}] Key="${key}" => either main record or offset record is null. Skipping.`);
                continue;
            }
            ...
            for (let fieldIndex = 0; fieldIndex < totalFields; fieldIndex++) {
                ...
                if (end > jsonStr.length || start > end) {
                    console.warn(`   - Field #${fieldIndex} path="${fieldInfo.path}" has out-of-range offsets [${start},${end}] for JSON length=${jsonStr.length}.`);
                    continue;
                }
                // Grab the actual substring from the JSON
                const substring = jsonStr.substring(start, end);
                console.log(`   - Field #${fieldIndex}: path="${fieldInfo.path}", offsetRange=[${start},${end}), substring="${substring}"`);
            }
        }
